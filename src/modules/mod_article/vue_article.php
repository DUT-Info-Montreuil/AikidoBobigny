...
class VueArticle extends VueGenerique
{
	...
	public function menu()
	{
		...
		<a href = "index.php?module=article&action=formDelete">Supprimer un article</a>', '<br/>
		<a href = "index.php?module=article&action=gererCommentaire">Gerer les commentaires</a>', '<br/>';
	}

	public function formArticle()
	{
		echo '</br><form action="index.php?module=article&action=insertArticle" method="POST" enctype="multipart/form-data">
	...
	{
		echo '</br><form action="index.php?module=article&action=deleteArticle" method="POST">
	...
	{
		echo '</br><form action="index.php?module=article&action=articleRecherche" method="POST">
	...
	public function afficherRecherche($recherche)
	{
		if ($recherche) {
			foreach ($recherche as $row) {
				$titre = $row['titre'];
				$id = $row['ID_article'];
				echo "</br><a href=index.php?module=article&action=articleDetails&id=$id>$titre</a></br>";
			}
		} else {
			echo '</br><p>Aucun article est disponible sur cette date</p>';
		}
	}
	...
	public function articleDetails($rechercheDetails, $rechercheCom)
	{
		if ($rechercheDetails) {
			foreach (array($rechercheDetails) as $row) {
				echo '<img src = "data:image/jpg;base64,' . base64_encode($row['img_bin']) . '" width = "400px" height = "400px"/></br>
				</br>' . $row['titre'] . '</br>
				</br>' . $row['texte'] . '</br>';
			}
		}
		...
		foreach ($rechercheCom as $rowCom) {
			echo '</br>' . $rowCom['login'] . '</br>' . $rowCom['texte'] . '</br><p>écrit le ' . $rowCom['date_com'] . '</p>';
		}
	}
	...
	public function listeArticleCommentaire($gestionCommentaire)
	{
		if ($gestionCommentaire) {
			echo '</br><p>Sélectionner un article dont vous voulez gerer les commentaires :</p></br>';
			foreach ($gestionCommentaire as $row) {
				$titre = $row['titre'];
				$id = $row['ID_article'];
				echo '</br><a href=index.php?module=article&action=listeCommentaire&id=$id>$titre</a></br>';
			}
		}
	}
	...
	{
		if ($rechercheCom) {
			echo '</br><p>Formulaire commentaires pour supprimer : </p></br>';
	...
	{
		if ($rechercheCom) {
			echo '</br><p>Formulaire commentaires pour valider : </p></br>';
			...
			foreach ($rechercheCom as $row) {
				...
				$statut = $row['com_validation'];
				echo '</br><tr>
						<td><input type= \'checkbox\' name =\'valider[]\' value =' . $id . '></td>
						<td>' . $auteur . '</td>
						<td>' . $texte . '</td>
						<td>' . $date . '</td>
						<td>' . $statut . '</td>
